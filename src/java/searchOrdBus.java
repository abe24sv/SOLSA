...
public class searchOrdBus extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(con!=null){
            ...
            try{
                PreparedStatement query=con.prepareStatement("SELECT * FROM (((compra_prod JOIN producto ON compra_prod.id_producto = producto.id) JOIN categoria ON producto.id_categoria = categoria.id) JOIN subcategoria ON producto.id_subcategoria = subcategoria.id) JOIN marca ON producto.id_marca = marca.id");
               ...
               HttpSession session=request.getSession();
                ...
                PreparedStatement query3=con.prepareStatement("SELECT * FROM (((compra_prod JOIN producto ON compra_prod.id_producto = producto.id) JOIN categoria ON producto.id_categoria = categoria.id) JOIN subcategoria ON producto.id_subcategoria = subcategoria.id) JOIN marca ON producto.id_marca = marca.id AND (producto.nombre LIKE ? OR producto.id_subcategoria LIKE ? OR prodcuto.id_categoria LIKE ? OR producto.id_marca LIKE ? );");
               query3.setString(1,"%" + busqueda + "%");
               query3.setString(2,"%" + busqueda + "%");
               query3.setString(3,"%" + busqueda + "%");
               query3.setString(4,"%" + busqueda + "%");
