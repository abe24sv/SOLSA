...
public class searchAdvSales extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String bcliente = request.getParameter("bcliente");
        String busuario = request.getParameter("busuario");
        String bvendedor = request.getParameter("bvendedor");
        String bproducto = request.getParameter("bproducto");
        ...
        if(con!=null){
            ...
            try{
               ...
               query3.close();
               PreparedStatement query4=con.prepareStatement("SELECT * FROM (((venta JOIN compra ON venta.id_compra = compra.id) JOIN cliente ON compra.id_cliente = cliente.id) JOIN usuario ON compra.id_usuario = usuario.id),usuario AS vendedor WHERE venta.id_usuario = vendedor.id AND (compra.estado = 1 OR compra.estado > 2);");
               result=query4.executeQuery();
               while(result.next()){
                   boolean b = false;
                   if(type.equals("OR")){
                        for(Producto p : compraProducto.get(result.getInt("venta.id_compra"))){
                            if(p.getnombre().toLowerCase().contains(bproducto.toLowerCase()) && !a.contains(result.getInt("venta.id_compra"))){
                                b = true;
                                break;
                            }
                        }
